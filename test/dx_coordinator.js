...
contract("Coordinator", (accounts) => {
  describe("participateInAuction()", () => {
  	it("updates auctionCount and cummulative shares after pinging from Coordinator", async() => {
      ...
      await instance2.deposit(20)
      await coordinator.participateInAuction()

      assert.equal(await instance1.auctionCount.call(), 1)
      assert.equal(await instance1.totalPoolSharesCummulative.call(), 10)

      assert.equal(await instance2.auctionCount.call(), 1)
      assert.equal(await instance2.totalPoolSharesCummulative.call(), 20)
    })
  })
})